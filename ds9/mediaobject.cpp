...
namespace Phonon
{
    namespace DS9
    {
        ...
        MediaObject::MediaObject(QObject *parent) : BackendNode(parent),
            ...
            m_curentAudioChannelIndex(0),
            m_curentSubtitleIndex(0),
            m_audioChanels(),
            m_subtitles(),
        ...
        void MediaObject::switchToNextSource()
        {
            ...
            setAudioChannels(currentGraph()->audioChannels());
            setSubtitles(currentGraph()->subtitels());
#ifndef QT_NO_PHONON_MEDIACONTROLLER
        ...
        {
            qDebug() << "_iface_setCurrentAudioChannel" << channel;
            int index = m_audioChanels.indexOf(channel);
            qDebug() << "_iface_setCurrentAudioChannel indexOf" << index;
            if (index >= 0){
                int globalStreamIndex = channel.property("globalIndex").value<int>();
                currentGraph()->setCurrentAudioChannel(globalStreamIndex);
                m_curentAudioChannelIndex = index;
            }

        }

        void MediaObject::setAudioChannels(const QList<AudioChannelDescription> &chanels)
        {
            const bool emitSignal = m_audioChanels.count() != chanels.count();
            qDebug() << "setAudioChannels" << emitSignal;
            m_audioChanels = chanels;
            if (emitSignal) {
        ...
        {
            qDebug() << "_iface_setCurrentSubtitle" << subtitle;
            int index = m_subtitles.indexOf(subtitle);
            qDebug() << "_iface_setCurrentSubtitle indexOf" << index;
            if (index >= 0){
                int globalStreamIndex = subtitle.property("globalIndex").value<int>();
                currentGraph()->setCurrentAudioChannel(globalStreamIndex);
                m_curentSubtitleIndex = index;
            }
        }

        void MediaObject::setSubtitles(const QList<SubtitleDescription> &subtitiles)
        {
            const bool emitSignal = m_subtitles.count() != subtitiles.count();
            qDebug() << "setSubtitles" << emitSignal;
