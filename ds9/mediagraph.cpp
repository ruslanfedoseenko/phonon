...
*/
        ...
        QList<AudioChannelDescription> MediaGraph::audioChannels()
        {
            QList<AudioChannelDescription> res;

                try{
                    ComPointer<IAMStreamSelect> pStreamSelect(m_realSource,IID_IAMStreamSelect);
                    if (pStreamSelect!= NULL) {
                        ...
                        for (int j =0 ; j < streamCount; j++){
                            pStreamSelect->Info(j,&ppmt, &dwFlags, &lcid, &dwGroup, &pszName, &pObject, &pUnk);
                            QString streamName = QString::fromWCharArray(pszName);
#ifdef STREAM_DEBUG
                            qDebug() << "GROUP:" << dwGroup << "Name:" << streamName ;
#endif
                            if (ppmt != NULL) {
                                if (ppmt->majortype == MEDIATYPE_Audio){
                                    QHash<QByteArray, QVariant> properties;
                                    properties["name"] = streamName;
                                    properties["description"] = "Audio Channel";
                                    properties["globalIndex"] = j;
                                    res.append(AudioChannelDescription(j,properties));
                                }
#ifdef STREAM_DEBUG
                                qDebug() << "MajorType:" << QUuid(ppmt->majortype) << "SubType:" << QUuid(ppmt->subtype) <<
                                            "FormatType:" << QUuid(ppmt->formattype);
#endif
                            } else {
#ifdef STREAM_DEBUG
                                qDebug() << "AM_MEDIA_TYPE param is NULL";
#endif
                            }
                    ...
                    qDebug() << "Exception in MediaGraph::audioChannels" << ex.what();
                }

            return res;
        }

        void MediaGraph::setCurrentAudioChannel(int index)
        {
             ComPointer<IAMStreamSelect> pStreamSelect(m_realSource,IID_IAMStreamSelect);
             pStreamSelect->Enable(index,AMSTREAMSELECTENABLE_ENABLE);
        }

        QList<SubtitleDescription> MediaGraph::subtitels()
        {
            QList<SubtitleDescription> res;
            int subtitileCount = 0;
...
#endif
                            if (ppmt != NULL) {
                                if (ppmt->majortype == MEDIATYPE_Subtitle){
                                    QHash<QByteArray, QVariant> properties;
                                    properties["name"] = streamName;
                                    properties["description"] = "Subtitle";
                                    properties["globalIndex"] = j;
                                    res.append(SubtitleDescription(j,properties));
                                }
#ifdef STREAM_DEBUG
                                qDebug() << "MajorType:" << QUuid(ppmt->majortype) << "SubType:" << QUuid(ppmt->subtype) <<
                                            "FormatType:" << QUuid(ppmt->formattype);
        ...
        void MediaGraph::setCurrentSubttile(int index)
        {
            ComPointer<IAMStreamSelect> pStreamSelect(m_realSource,IID_IAMStreamSelect);
            pStreamSelect->Enable(index,AMSTREAMSELECTENABLE_ENABLE);
        }
